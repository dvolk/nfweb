...
def getDBConn(pwd : pathlib.Path = pathlib.Path.cwd()):
    ...
    con.execute("CREATE TABLE if not exists nfruns (date_time, duration, code_name, status, hash, uuid, command_line, user, sample_group, workflow, context, root_dir, run_uuid primary key not null, start_epochtime, pid, ppid, end_epochtime);")
    con.commit()
    return con
...
def insertDummyRun(data: dict, db_dir: pathlib.Path = pathlib.Path.cwd()):
    ...
    con.execute('insert into nfruns values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)',
               ...
               data['context'], data['root_dir'] ,data['run_uuid'], 
